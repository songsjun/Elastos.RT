    ...
    for (n = 0; n < cParams; n++) {
        if (BT_IS_IN(pParams[n])) { // [in] or [in, out]
            switch (BT_TYPE(pParams[n])) {
                ...
                case BT_TYPE_CSTRING:
                    pParcel->WriteCString((const char*)*puArgs);
                    puArgs++;
                    break;
    ...
    for (n = 0; n < cParams; n++) {
        if (BT_IS_OUT(pParams[n])) {    // [out] or [in, out]
                    ...
                    case BT_TYPE_ARRAYOF:
                        ...
                        *puArgs = (UInt32)(byte *)puOutBuffer;
                        ...
                        if ((BT_TYPE(pParams[n]) == BT_TYPE_STRINGBUF)) {
                            ((PCARQUINTET)*puArgs)->m_used = sizeof(Int8);
                            *(Int8 *)(((PCARQUINTET)*puArgs)->m_pBuf) = 0;
                        }
            ...
            switch (BT_TYPE(pParams[n])) {
                ...
                case BT_TYPE_CSTRING:
                    pParcel->ReadCStringPtr((Handle32*)puArgs);
                    puArgs++;
                    break;
