    ...
    switch(type) {
        ...
        case Type_CStringPtr:
            if (*m_elemPtr == MSH_NOT_NULL) {
                ...
                *(CString**)pValue = new CString(
                    CString::Duplicate((char*)m_elemPtr));
                assert(len == (Int32)MSH_ALIGN_4(strlen((char*)m_elemPtr) + 1));
                m_elemPtr += len / 4;
            }
            else {
        ...
        case Type_StringPtr:
            if (*m_elemPtr == MSH_NOT_NULL) {
                ...
                String* str = new String((char*)m_elemPtr);
                *(String**)pValue = str;
                assert(len == (Int32)MSH_ALIGN_4(strlen((char*)m_elemPtr) + 1));
                m_elemPtr += len / 4;
            }
...
ECode CRemoteParcel::ReadCStringPtr(
    /* [out] */ Handle32 *cspp)
{
    if (cspp == NULL) return E_INVALID_ARGUMENT;

    return ReadValue((PVoid)cspp, Type_CStringPtr);
}
...
ECode CRemoteParcel::WriteChar8Ptr(
    /* [in] */ Handle32 addr)
{
    return WriteValue((PVoid)addr, Type_Char8Ptr, sizeof(Int8*));
}
